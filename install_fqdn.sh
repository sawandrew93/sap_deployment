...
# Get the Fully Qualified Domain Name (FQDN) to be used for HANA and SAP
FQDN=$(hostname -f)
...
if su - $SID_USER -c "HDB version" &>/dev/null; then
    ...
    if [[ -d "$hana_db_dir" ]]; then
        ...
        cd "$hana_db_dir"
        # MODIFIED HERE: Added --hostname="$FQDN" to force FQDN during installation
...
# MODIFIED HERE: Replaced $(hostname) with $FQDN
