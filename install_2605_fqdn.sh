...
HDB_PARAM_FILE="$SCRIPT_DIR/hdb_param.cfg"
SAP_PARAM_FILE="$SCRIPT_DIR/sap_param.cfg"

if [[ ! -f "$HDB_PARAM_FILE" ]]; then
    echo "❌ Error: hdb_param.cfg missing from script directory ($SCRIPT_DIR)." | tee -a "$LOGFILE"
    exit 1
fi

if [[ ! -f "$SAP_PARAM_FILE" ]]; then
    echo "❌ Error: sap_param.cfg missing from script directory ($SCRIPT_DIR)." | tee -a "$LOGFILE"
    exit 1
fi
...
if [[ "$user_choice" == "2" ]]; then
    ...
    # Find and extract all ZIP files recursively, skipping *Integration.zip and *AddOns.zip
    ...
    SAPCAR_RAR="$SCRIPT_DIR/tools/SAPCAR_0-80000935.rar"
    SAPCAR_DIR="$SCRIPT_DIR/tools/sapcar"

    if [[ ! -f "$SAPCAR_RAR" ]]; then
        echo "❌ SAPCAR archive not found: $SAPCAR_RAR" | tee -a "$LOGFILE"
        exit 1
    fi

    mkdir -p "$SAPCAR_DIR"

    # Check whether SAPCAR executable is already extracted
    SAPCAR_EXE=$(find "$SAPCAR_DIR" -type f -iname "SAPCAR_*.EXE" -print -quit)

    # Extract SAPCAR only if executable is not already available
    if [[ -z "$SAPCAR_EXE" ]]; then
        echo "SAPCAR executable not found. Extracting SAPCAR archive..." | tee -a "$LOGFILE"
        unar -f -o "$SAPCAR_DIR" "$SAPCAR_RAR" 2>&1 | tee -a "$LOGFILE"

        if [[ ${PIPESTATUS[0]} -ne 0 ]]; then
            echo "❌ Failed to extract SAPCAR archive." | tee -a "$LOGFILE"
            exit 1
        fi

        # Find extracted SAPCAR executable
        SAPCAR_EXE=$(find "$SAPCAR_DIR" -type f -iname "SAPCAR_*.EXE" -print -quit)
    fi

    if [[ -z "$SAPCAR_EXE" || ! -f "$SAPCAR_EXE" ]]; then
        echo "❌ SAPCAR executable not found after extraction." | tee -a "$LOGFILE"
        exit 1
    fi

    chmod +x "$SAPCAR_EXE"
    echo "✅ Using SAPCAR: $SAPCAR_EXE" | tee -a "$LOGFILE"

    # Find all SAR files and extract them
    find . -type f -iname "*.SAR" -print0 | while IFS= read -r -d '' sar_path; do
        sar_dir=$(dirname "$sar_path")
        sar_file=$(basename "$sar_path")

        echo "Extracting $sar_file with SAPCAR..." | tee -a "$LOGFILE"

        (
            cd "$sar_dir" || {
                echo "❌ Failed to cd to $sar_dir" | tee -a "$LOGFILE"
                exit 1
            }

            "$SAPCAR_EXE" -manifest SIGNATURE.SMF -xvf "$sar_file" 2>&1 | tee -a "$LOGFILE"

            if [[ ${PIPESTATUS[0]} -ne 0 ]]; then
                echo "❌ Failed to extract $sar_file" | tee -a "$LOGFILE"
                exit 1
            fi

            chmod -R +x .
            # Remove SAR after successful extraction
            rm -f "$sar_file"
        )
    done

    echo "✅ All SAR files have been extracted." | tee -a "$LOGFILE"
else
...
PACKAGES="jq libatomic1 rpm-build xmlstarlet python3-pyOpenSSL bc glibc-i18ndata \
          libcap-progs libicu60_2 insserv-compat nfs-kernel-server libidn11 samba"
...
sed -i "s|sap_admin_pw|${NEW_DB_USER_PW}|g" /tmp/hdb.cfg # change sap_adm_pw
sed -i "s|system_pw|${SYSTEM_USER_PW}|g" /tmp/hdb.cfg    # change system database user password
sed -i "s|NDB|${SID}|g" /tmp/hdb.cfg                     # change sid value
...
# Install hana database, afl and client (Skipped if already installed)
...
if su - $SID_USER -c "HDB version" &>/dev/null; then
    ...
    if [[ -d "$hana_db_dir" ]]; then
        ...
        cd "$hana_db_dir" || exit 1
        ./hdblcm --batch --configfile="/tmp/hdb.cfg" --hostname="$FQDN" 2>&1 | tee -a "$LOGFILE"
        if [[ $? -eq 0 ]]; then
...
GRANT \"BACKUP ADMIN\" TO ${NEW_DB_USER};
GRANT \"BACKUP OPERATOR\" TO ${NEW_DB_USER};
GRANT \"CREATE SCHEMA\",\"USER ADMIN\",\"ROLE ADMIN\",\"CATALOG READ\",\"CERTIFICATE ADMIN\",\"TRUST ADMIN\" TO ${NEW_DB_USER} WITH ADMIN OPTION;
...
# Get the second line as version is included in 2nd line
...
if [[ "$clean_installed" == "$clean_installer" ]] && [[ -n "$clean_installed" ]]; then
